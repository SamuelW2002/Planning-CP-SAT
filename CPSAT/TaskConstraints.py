...
# The amount a order is planned passed it's due date is max half a year after, can be reduced further
MAX_PAST_DUE_DATE = 15778800
 
def add_task_vs_task_penalties(model_data, interval_data_1, machine_task_intervals, machine_id, logger):
    with logger.context("Task vs Task Constraints"):
        ...
        logger.info("Looping through all of the tasks")

        for interval_data_2 in machine_task_intervals:
            with logger.context(f'Task: {interval_data_2["task_id"]}'):
                if interval_data_1 == interval_data_2:
                    logger.info("Task 1 and 2 are the same, skipping...")
                    continue

                logger.info("Created TaskInterval objects for task 1 and task 2")
                ...
                logger.info("Extracted intervals, order vars and task ids")

                model_data.model.Add(order_var_1 != order_var_2).OnlyEnforceIf(is_chosen_1, is_chosen_2)
                logger.info("Created constraint saying that task 1 is not allowed to have the same order as task 2")

                order_follow_var = model_data.model.NewBoolVar(f"order_follow_{task_1_id}_{task_2_id}")
                logger.info("Created boolean that indicates if task 2 comes after task 1")
                ...
                logger.info("Created constraints that say order folow boolean is true when task 2 order is task 1 order + 1")

                follow_and_chosen = model_data.model.NewBoolVar(f"follow_and_chosen_{task_1_id}_{task_2_id}")
                logger.info("Created boolean that indicates if both tasks are chosen and task 2 follows task 1")
                ...
                logger.info("Enforced constraint saying that follow and chosen boolean is true if both tasks are chosen and task 2 follows task 1")
                ...
                if interval_data_1["priority_code"] == 7 and not interval_data_2["priority_code"] == 7:
                    ...
                    logger.info("Task 1 is a stock fill task and task 2 is not a stock fill task, applying penalty if task 2 is scheduled after task 1")

                add_preparation_time_penalty(model_data, follow_and_chosen, machine_id, interval_data_1, interval_data_2, logger)


def add_preparation_time_penalty(model_data, follow_and_chosen, machine_id, interval_data_1, interval_data_2 , logger):
    task_1_id = interval_data_1["task_id"]
    task_2_id = interval_data_2["task_id"]
    ...
    if setup_penalty_seconds == 0:
        logger.info("No swap was needed between these 2 tasks")
        return
    
    logger.info(f"Calculated that penalty is {setup_penalty_seconds} seconds and of type {setup_penalty_type}")

    prep_start_var = model_data.model.NewIntVar(0, model_data.max_ceiling_variable, f'prep_start_{task_1_id}_to_{task_2_id}')
    logger.info("Defined variable that represents the start of the preparation time between task 1 and 2")
    ...
    logger.info("Created optional interval that is present if follow and chosen between task 1 and 2 is true with a duration equal to the penalty time")
    ...
    logger.info("Enforced constraint saying that the preparation can only start after task 1 ends")
    ...
    logger.info("Enforced constraint saying task 2 can only start after preparation ends")
    ...
    logger.info("Created a dict for this interval needed for extraction")

    if setup_penalty_type == "ombouw":
        logger.info("Identified subserie swap: preparing for cumulative constraints.")
    ...
    logger.info("Added preparation interval to the list of all prep intervals for no overlap constraint")
...
def add_deadline_slack_constraint(model_data, interval_data, logger):
    with logger.context("Deadline Slack Constraint"):
        ...
        deadline_slack_var = model_data.model.NewIntVar(0, due_date_seconds + MAX_PAST_DUE_DATE * 2, f'deadline_slack_{task_id}')
        logger.info("Created deadline slack var that has to be between 0 and a year after the due date")
...
def add_makespan_constraint(model_data, interval_data, logger):
    with logger.context("Makespan constraint"):
        ...
        logger.info("Defined variable that represents the chosen end var")

        model_data.model.Add(chosen_end_var == interval.EndExpr()).OnlyEnforceIf(is_chosen)
        logger.info("Added constraint saying if the task is chosen then the chosen end var is equal to the end var")
        ...
        if priority_code == 1: 
            ...
            logger.info("Added machine to list to track what machines were used for emergency machine")

        elif priority_code == 2:
            logger.info("Task has a priority level of 2")
            model_data.currently_running_tasks.append(interval_data)
            logger.info("Added task to list of orders that are currently being produced")

        elif priority_code == 3:
            logger.info("Task has priority code 3")
            model_data.all_penalties.append(chosen_end_var * 50)
            logger.info("Added penalty of the chosen end var multiplied by 50, making the solver prioritize this task but wont interupt any already running tasks")

        elif priority_code == 4:
            logger.info("Task has priority code 4")
            model_data.all_penalties.append(deadline_slack * 20)
            logger.info("Task is not a priority but should not miss deadline, penalty = how much deadline is missed in seconds * 5")
...
def add_subserie_unavailable_constraint(model_data, interval_data, dataframes, logger):
    with logger.context("Subserie Unavailable Constraint"):
        ...
        for index, window in relevant_windows_df.iterrows():
            ...
            with logger.context(f"{start_dt} - {end_dt}"):
                logger.info(f"Creating interval for range with start {start_dt} and end {end_dt}")

                block_start = DTS(start_dt, logger)
                block_end = DTS(end_dt, logger)
                logger.info(f"Start in seconds from now: {block_start}")
                logger.info(f"End in seconds from now: {block_end}")

                ends_before_block = model.NewBoolVar(f'ebb_{task_id}_win{index}')
                logger.info("Defined variable used for indicating if task ends before the block")
                ...
                logger.info("Added constraint saying end before block bool is true if task ends before block starts")

                starts_after_block = model.NewBoolVar(f'sab_{task_id}_win{index}')
                logger.info("Defined variable used for indicating if task starts after the block")
                ...
                logger.info("Added constraint saying start after block bool is true if task starts after block ends")

                no_overlap_condition = model.NewBoolVar(f'no_ov_{task_id}_win{index}')
                logger.info("Created bool var representing no overlap between block and task")
                ...
                logger.info("Added constraint saying no overlap occurs if the task starts after the block or ends before the block")

                model.AddImplication(interval_data["is_chosen"], no_overlap_condition).WithName(f'subserie_no_overlap_{task_id}_win{index}')
                logger.info("Added constraint saying if the task is chosen then no overlap can occur")
