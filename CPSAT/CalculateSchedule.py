...
from CPSAT.PrepareIntervalsFunctions import create_intervals_for_orders, create_capacity_reduction_intervals, calculate_allowed_prep_range_sub_swap
...
def create_optimized_schedule_df(dataframes, logger, duration):
    with logger.context("Creating Optimal Schedule"):
        ...
        create_intervals_for_orders(model_data, possible_machines_for_orders_df, logger, MAX_HORIZON_DATE)
        create_capacity_reduction_intervals(model_data, ombouwers_beschikbaar_df, DEFAULT_SUB_SWAP_AMOUNT_CAP, logger, MAX_HORIZON_DATE)
        calculate_allowed_prep_range_sub_swap(model_data, dataframes, logger, MAX_HORIZON_DATE)
        ...
        # Allow a worker to restart when finding to many infeasible solutions so that it does not get stuck in a infeasible region
        #solver.parameters.feasibility_jump_enable_restarts =  True
            ...
            with logger.context(f"Machine: {machine_id}"):
                ...
                for interval_data in machine_task_intervals:
                    with logger.context(f'Task: {interval_data["task_id"]}'):
                        add_deadline_slack_constraint(model_data, interval_data, logger, MAX_HORIZON_DATE)
