...
def create_intervals_for_orders(model_data, possible_machines_for_orders_df, logger, MAX_HORIZON_DATE):
            ...
            with logger.context(f"Interval for order: {order_id}"):
                ...
                for index, task in all_possible_tasks_for_order.iterrows():
                    with logger.context(f"Task {index}"):
                        ...
                        # x 2 anders geeft hij infeasible en ik zie niet direct waarom
                        start_var = model_data.model.NewIntVar(0, MAX_HORIZON_DATE * 2, f'start_{interval_data["task_id"]}')
                        duration_var = model_data.model.NewConstant(interval_data['duration'])
                        end_var = model_data.model.NewIntVar(0, MAX_HORIZON_DATE * 2, f'end_{interval_data["task_id"]}')
...
def create_capacity_reduction_intervals(model_data, ombouwers_beschikbaar_df, default_capacity, logger, MAX_HORIZON_DATE):
    with logger.context("Capacity Reduction Intervals"):
        ...
        horizon_marker_date = (now + timedelta(seconds=MAX_HORIZON_DATE)).date()
...
def calculate_allowed_prep_range_sub_swap(model_data, dataframes, logger, MAX_HORIZON_DATE):
    with logger.context("Subserie Swap Time Ranges"):
        ...
        horizon_days = math.ceil(MAX_HORIZON_DATE / (24 * 3600)) + 1
